...
export default function MessageList({ messages }: MessageListProps) {
  ...
  useEffect(() => {
    ...
    // 2. New messages arrived and they're from the current user
    const shouldAutoScroll = 
      isScrolledToBottom || 
      (hasNewMessages && messages[messages.length - 1]?.senderId === currentUser?.id);
              ...
              if (message.isDeleted) {
                return (
                  ...
                  >
                    ...
                    <div className={`flex-1 max-w-2xl ${message.senderId === currentUser?.id ? 'text-right' : ''}`}>
                      <div className="inline-block bg-secondary-50 dark:bg-secondary-800/50 rounded-lg px-4 py-2">
