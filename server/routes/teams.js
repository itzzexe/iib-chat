...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    logger.error('Get teams error', { error: error.message, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
...
router.get('/:id', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Get team error', { error: error.message, teamId: req.params.id, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    logger.error('Create team error', { error: error.message, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.put('/:id', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Update team error', { error: error.message, teamId: req.params.id, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.post('/:id/members', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Add team member error', { error: error.message, teamId: req.params.id, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.delete('/:id/members/:userId', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Remove team member error', { error: error.message, teamId: req.params.id, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.delete('/:id', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Delete team error', { error: error.message, teamId: req.params.id, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
