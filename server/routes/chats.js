...
router.get('/oversee/direct', authenticateToken, requireManager, async (req, res) => {
  try {
    ...
    logger.error('Oversee direct chats error', { error: error.message, userId: req.user.userId });
    res.status(500).json({ 
      success: false,
...
router.get('/oversee/:id/messages', authenticateToken, requireManager, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Oversee messages error', { error: error.message, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.delete('/:chatId/messages', authenticateToken, requireManager, async (req, res) => {
  try {
    ...
    logger.info('Clear chat messages request', { chatId, userId: req.user.userId });
    
    // Validate chatId format
    if (!mongoose.Types.ObjectId.isValid(chatId)) {
      logger.warn('Invalid chatId format', { chatId, requesterId: req.user.userId });
    ...
    if (!chat) {
      logger.warn('Chat not found for clearing', { chatId, requesterId: req.user.userId });
    ...
    logger.info('Chat found for clearing', { chatId, chatType: chat.type, chatName: chat.name, requesterId: req.user.userId });
    
    // Clear all messages in the chat
    const result = await Message.deleteMany({ chatId });
    logger.info('Messages cleared', { chatId, deletedCount: result.deletedCount, requesterId: req.user.userId });
    ...
    try {
      ...
      logger.error('Failed to log action', { error: logError.message, action: 'chat.cleared', chatId });
    }
    ...
    logger.error('Clear chat error', { error: error.message, chatId: req.params.chatId, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false, 
...
router.delete('/:chatId', authenticateToken, requireManager, async (req, res) => {
  try {
    ...
    logger.info('Delete chat request', { chatId, userId: req.user.userId });
    ...
    const chat = await Chat.findById(chatId);
    if (!chat) {
      logger.warn('Chat not found', { chatId, requesterId: req.user.userId });
      return res.status(404).json({ 
        success: false, 
        error: 'Chat not found' 
      });
    ...
    logger.info('Chat found', { chatId, chatType: chat.type, chatName: chat.name, requesterId: req.user.userId });
    
    // Prevent deletion of system chats
    if (chat.type === 'system') {
      logger.warn('Attempted to delete system chat', { chatType: chat.type, requesterId: req.user.userId });
    ...
    logger.info('Messages deleted', { chatId, deletedCount: deleteResult.deletedCount, requesterId: req.user.userId });
    
    // Delete the chat itself
    await Chat.findByIdAndDelete(chatId);
    logger.info('Chat deleted successfully', { chatId, requesterId: req.user.userId });
    ...
    try {
      ...
      logger.error('Failed to log action', { error: logError.message, action: 'chat.deleted', chatId });
    }
    ...
    logger.error('Delete chat error', { error: error.message, chatId: req.params.chatId, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false, 
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    logger.error('Get chats error', { error: error.message, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.get('/:id', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Get chat error', { error: error.message, chatId: req.params.id, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.post('/', authenticateToken, validateChat, async (req, res) => {
  try {
    ...
    logger.error('Create chat error', { error: error.message, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.put('/:id', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Update chat error', { error: error.message, chatId: req.params.id, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.patch('/:id/archive', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Archive chat error', { error: error.message, chatId: req.params.id, requesterId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.get('/:id/messages', authenticateToken, validateObjectId(), async (req, res) => {
  try {
    ...
    logger.error('Get messages error', { error: error.message, chatId: req.params.id, userId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.post('/:id/messages', messageRateLimit, authenticateToken, validateObjectId(), validateMessage, async (req, res) => {  try {
    ...
    logger.error('Send message error', { error: error.message, chatId: req.params.id, userId: req.user.userId });
    res.status(500).json({ 
      success: false,
      error: 'Internal server error' 
    });
...
router.put('/:chatId/messages/:messageId', authenticateToken, validateObjectId('chatId'), validateObjectId('messageId'), async (req, res) => {
  try {
    ...
    logger.error('Update message error', { error: error.message, messageId: req.params.messageId, userId: req.user.userId });
...
router.delete('/:chatId/messages/:messageId', authenticateToken, validateObjectId('chatId'), validateObjectId('messageId'), async (req, res) => {
  try {
    ...
    logger.error('Delete message error', { error: error.message, messageId: req.params.messageId, userId: req.user.userId });
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});
...
router.post('/:chatId/messages/read', authenticateToken, validateObjectId('chatId'), async (req, res) => {
  try {
    ...
    logger.error('Mark messages as read error', { error: error.message, chatId: req.params.chatId, requesterId: req.user.userId });
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});
